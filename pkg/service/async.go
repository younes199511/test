			...
			for _, cv := range acv.CommandValues {
				...
				if common.CurrentConfig.Device.DataTransform {
					...
					if err != nil {
						...
						cv = dsModels.NewStringValue(cv.DeviceResourceName, cv.Origin, fmt.Sprintf("Transformation failed for device resource, with value: %s, property value: %v, and error: %v", cv.String(), dr.Properties.Value, err), 5)
					}
				}
