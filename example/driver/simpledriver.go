...
func (s *SimpleDriver) HandleReadCommands(deviceName string, protocols map[string]contract.ProtocolProperties, reqs []dsModels.CommandRequest) (res []*dsModels.CommandValue, err error) {
	...
	if len(reqs) == 1 {
		...
		if reqs[0].DeviceResourceName == "SwitchButton" {
			cv, _ := dsModels.NewBoolValue(reqs[0].DeviceResourceName, now, s.switchButton,2)
			res[0] = cv
		} else if reqs[0].DeviceResourceName == "Xrotation" {
			cv, _ := dsModels.NewInt32Value(reqs[0].DeviceResourceName, now, s.xRotation,3)
			res[0] = cv
		} else if reqs[0].DeviceResourceName == "Yrotation" {
			cv, _ := dsModels.NewInt32Value(reqs[0].DeviceResourceName, now, s.yRotation,4)
			res[0] = cv
		} else if reqs[0].DeviceResourceName == "Zrotation" {
			cv, _ := dsModels.NewInt32Value(reqs[0].DeviceResourceName, now, s.zRotation,6)
			res[0] = cv
			...
			cvb, _ := dsModels.NewBinaryValue(reqs[0].DeviceResourceName, now, buf.Bytes(),5)
			res[0] = cvb
		} else if reqs[0].DeviceResourceName == "Uint8Array" {
			cv, _ := dsModels.NewUint8ArrayValue(reqs[0].DeviceResourceName, now, []uint8{0, 1, 2},23)
			res[0] = cv
		}
	} else if len(reqs) == 3 {
		...
		for i, r := range reqs {
			...
			case "Xrotation":
				cv, _ = dsModels.NewInt32Value(r.DeviceResourceName, now, s.xRotation,22)
			case "Yrotation":
				cv, _ = dsModels.NewInt32Value(r.DeviceResourceName, now, s.yRotation, 26)
			case "Zrotation":
				cv, _ = dsModels.NewInt32Value(r.DeviceResourceName, now, s.zRotation, 78)
			}
			res[i] = cv
		}
	}
